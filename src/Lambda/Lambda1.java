package Lambda;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

public class Lambda1 {
    public static void main(String[] args) {

/*
   1) Lambda "Functional Programming"-->mathod(action) kullanma pr dili.
      Lambda --> mathod create  etme değil mevcut method'ları(library)secip kullanmaktır...
      Lambda  kendi başına tanımlanabilen parametre alıp gönderebilen fonksiyonlardır.
      Lambda'lar sayesinde daha az kod ve hızlı geliştirme sağlanabilmektedir.
      Java 8 ile gelen bu özellik, Java 8’in en önemli özelliğidir.

      "Functional Programming" de "Nasil yaparim?" degil "Ne yaparim?" dusunulur.
   2) "Structured Programming" de "Ne yaparim?" dan cok "Nasil Yaparim?" dusunulur
   3) "Functional Programming" hiz, code kisaligi, code okunabilirligi
       ve hatasiz code yazma acilarindan cok faydalidir.
   4) Lambda sadece collections'larda(List, Queue ve Set) ve array'lerde kullanilabilir ancak map'lerde kullanılmaz.
      Lambda kullanmak hatasız code kullanmaktır.

          Collections Nedir?
          Çoğu yazılım tek tek öğeler yerine öğelerden oluşan toplulukları depolar ve onlar üzerinde işlem
          yapar. Array'ler onlardan birisidir. Java Collections Framework, arraylerle yapılan işleri daha kolay
          yaptığı gibi, daha fazlasını da yapar.
          Java'da bir koleksiyon (collection - bazen container, ambar diye adlandırılır) nesnelerden oluşan bir
          topluluğu bir arada tutan bir yapıdır. 'Collections Framework' ise arayüzler ve onların kurgularından
          (implementations) oluşur.
*/

        List<Integer> sayi = new ArrayList<>(Arrays.asList(34, 22, 16, 11, 35, 20, 63, 21, 65, 44, 66, 64, 81, 38, 15));

        // Task: Structur Programing kullanarak list elemanlatini bosluk olacak sekilde ayni satirdaprint edin


        printElStructured(sayi);
        printElFunktional(sayi);
        printElFunktional1(sayi);
        printElFunktional2(sayi);
        System.out.println("");
        ciftleriFiltrelemeStructured(sayi);
        System.out.println("");
        ciftleriFiltrelemeFunktional(sayi);
        System.out.println("");
        ciftleriFiltrelemeFunktional1(sayi);
        System.out.println("");
        ciftleriFiltrelemeFunktional2(sayi);

    }

    private static void printElStructured(List<Integer> sayi) {
        for (Integer w : sayi) {
            System.out.print(w + " ");
        }

        // Task: Lambda(Funktional) Programing kullanarak list elemanlatini bosluk olacak sekilde ayni satirdaprint edin

      /*
 stream() : datalari yukaridan asagiya akis sekline getirir. Stream bir interface olduğundan dolayı doğrudan nesne almaz.
 forEach() :datanin parametresine gore her bir elemanı isler
 t-> : Lambda operatoru
  Lambda Expression-->(parameter list) -> {action body}
      Parameter list: Fonksiyonun parametreleri tanımlanır. Hiç parametre geçirmeden boşta olabilir.
      -> Arrow-token: Argüman listesi ile expression gövdesini birbirine bağlamak için kullanılır.
      Body: Expressionları ve statementları içerir.

     Bir kod bloğundan oluşan bir body...
     Bu tip lambda body, block body olarak bilinir. Blok gövdesi, lambda gövdesinin birden çok ifade içermesine izin verir.
     Bu ifadeler parantez içine alınır ve parantezlerden sonra noktalı virgül eklemeniz gerekir.

         () -> {
          double pi = 3.1415;
             return pi;
         };
  Lambda Expression  yapisi cok tavsiye edilmez daha cok METHOD REFERENCE kullanilir

 */

    }
    private static void printElFunktional(List<Integer> sayi) {
        System.out.println("");
        sayi.stream().forEach(t -> System.out.print(t + " "));
    }
    private static void printElFunktional1(List<Integer> sayi) {
        System.out.println("");
        sayi.stream().forEach(System.out::print);
    }

    public static void yazdir(int a){
        System.out.print(a+ " ");
    }

    private static void printElFunktional2(List<Integer> sayi) {
        System.out.println("");
        sayi.stream().forEach(Lambda1::yazdir);
    }

    //structured Programming ile list elemanlarinin  cift olanalrini
    // ayni satirda aralarina bosluk birakarak print ediniz

    public static void ciftleriFiltrelemeStructured(List<Integer> sayi){
        for (Integer each: sayi) {
            if (each%2==0){
                System.out.print(each+" ");
            }
        }
    }

    //funktional Programming ile list elemanlarinin  cift olanalrini
    // ayni satirda aralarina bosluk birakarak print ediniz

    public static void ciftleriFiltrelemeFunktional(List<Integer> sayi){
        sayi.stream().filter(t-> t%2==0).forEach(Lambda1::yazdir);

    }

    public static boolean ciftBul(int a){

        return a%2==0;
    }

    public static void ciftleriFiltrelemeFunktional1(List<Integer> sayi){
        sayi.
                stream().
                filter(Lambda1::ciftBul).
                forEach(Lambda1::yazdir);

    }

    //Task : functional Programming ile list elemanlarinin 34 den kucuk cift olanalrini
    // ayni satirda aralarina bosluk birakarak print ediniz.

    public static boolean ciftBulOtuzdorttenKucuk(int a){
        return a<34;
    }

    public static void ciftleriFiltrelemeFunktional2(List<Integer> sayi){
        sayi.
                stream().
                filter(Lambda1::ciftBul).
                filter(Lambda1::ciftBulOtuzdorttenKucuk).
                forEach(Lambda1::yazdir);

    }




}
